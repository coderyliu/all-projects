<template>
  <div class="rs">
    ...
    <el-breadcrumb separator=">">
      <el-breadcrumb-item :to="{ path: '/doctorCenter' }"
    ...
    <el-card class="box-card">
      ...
      <!-- 用户列表区 -->
      <el-table :data="doctorList" style="width: 100%" border stripe>
    ...
    getDoctorList() {
      request({
        ...
        if (res.data.data.state === true) {
          // console.log(res)
          this.$message.success("获取医生列表成功!");
          this.doctorList = res.data.data["newArr"];
    ...
    async deleteDialog(phone) {
      ...
      if (confirmResult === "confirm") {
        request({
          ...
          this.getDoctorList();
          return this.$message.success("该医生已删除!");
        });
      } else {
        return this.$message.error("该医生取消删除");
      }
    },
    editClosed() {
    ...
    addUser() {
      this.$refs.addFormRef.validate((valid) => {
        ...
        request({
          ...
          //添加后重新获取用户数据，不需要手动刷新
          this.getDoctorList();
          return this.$message.success("医生添加成功了!");
        });
      });
    },
  },
